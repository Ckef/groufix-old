		...
		GFXProgram*     program)
{
	...
	unsigned char ext = GFX_CONT_GET.ext[GFX_EXT_PROGRAM_MAP];
		...
		GFXProgramMap* map)
{
	...
	/* Use all programs */
	if(GFX_CONT_GET.ext[GFX_EXT_PROGRAM_MAP] && !internal->blocks)
	{
		for(stage = 0; stage < GFX_INT_NUM_STAGES; ++stage)
		...
		GFXProgram*     share)
{
	...
	if(share)
	{
		...
		if(!_gfx_program_reference(share, 1))
			return 0;
	}
